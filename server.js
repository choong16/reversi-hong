    ...
    */
    ...
    socket.on('invite', (payload) => {
        ...
        io.in(room).allSockets().then((sockets)=> {
            /* invitee isn't the room */
            if ((typeof sockets == 'undefine') || (sockets === null) || !sockets.has(requested_user)){
                response = {
                    result:'fail',
...
or
    ...
    */
    ...
    socket.on('play_token', (payload) => {
        ...
        /** Make sure the current attempt is by the correct c color */
        ...
        /** Make sure the current play is coming form teh correct player */
...
function check_line_match(color, dr, dc, r, c, board) {
    ...
    /** Check to make sure we arne't going to walk off the board */
    if ((r + dr < 0) || (r + dr > 7)) {
        return false;
    }
    if ((c + dc < 0) || (c + dc > 7)) {
...
/** Return true if r + ddr supports play at r and same for column */
function adjacent_support(who, dr, dc, r, c, board) {
    ...
    if (who === 'b') {
        ...
        log("Houston we ahve a problem: " + who);
        return false;
    }
    ...
    io.of('/').to(game_id).allSockets().then((sockets) => {
        ...
        if (sockets.size >= 1) {
            ...
            if ((games[game_id].player_white.socket != first) &&
                ...
                /**player does not ahvea color */
                if (games[game_id].player_white.socket === "") {
                    /** This player should be white */
                    console.group("White is assigned to: " + first);
                    ...
                    console.group("Black is assigned to: " + first);
                    games[game_id].player_black.socket = first;
